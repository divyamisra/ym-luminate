...
(function($) {
	...
	var coordEmail = $('input[name=coordinator_email]').val();
        ...
        // Leader For Life 2020 Registration JS
...
//                    cd.getCompanyList(evID);
	...
	$('.input-container label:contains(Other Amount)').prev('input').andSelf().wrapAll('<div class="donation-level-row-decoration-container"></div>');
		...
		if (isProd) {
	                $('<img width="1" height="1" style="display:none;" src="https://www2.heart.org/site/SPageServer?pagename=reus_stem_mobileopt_add_group&group_id=' + $('body').data("group-id") + '&pgwrap=n" id="mobileopt_add_group">').appendTo($('#fr_reg_summary_page'));
		} else {
	                $('<img width="1" height="1" style="display:none;" src="https://dev2.heart.org/site/SPageServer?pagename=reus_stem_mobileopt_add_group&group_id=' + $('body').data("group-id") + '&pgwrap=n" id="mobileopt_add_group">').appendTo($('#fr_reg_summary_page'));
		}
    	    }
        }
  ...
  if ($('input[name="pg"]').val() == 'paymentForm') {
      ...
      var label = $(this).closest('.form-content').find('label').html();
       ...
       $('button.next-step').click(function(){
        ...
        $('.waiverCheck label').html('<span class="field-required"></span> I accept and acknowledge that I have read and understand this Leader For Life <a id="waiverPopLink" href="javascript:void(0);">Release with Publicity Consent</a> and agree to them voluntarily.');
	...
	$('.survey-question-container legend span:contains("Healthy for good signup")').parent().parent().addClass('healthyCheck');
        ...
        */
        ...
        jQuery("input[name='fr_part_radio']").blur(function() {
            _gaq.push(['t2._trackEvent', 'Participation Type', 'click', 'Leader For Life']);
        });
        jQuery("input[name='fr_part_radio']").blur(function() {
            _gaq.push(['t2._trackEvent', 'Participation Type', 'click', 'Leader For Life']);
        });
