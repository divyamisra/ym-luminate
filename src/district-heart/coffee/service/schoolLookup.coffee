  ...
  .factory 'SchoolLookupService', [
    ...
    ($rootScope, $http, $sce) ->
      ...
      getSchoolData: ->
        ...
        requestUrl += 'SPageServer?pagename=reus_ym_dhc_school_data_csv&pgwrap=n'
      ...
      getSchoolDataByState: (requestData) ->
        ...
        if window.location.protocol is 'https:'
          requestUrl = luminateExtend.global.path.secure + 'S'
        requestUrl += 'SPageServer?pagename=reus_ym_dhc_school_data_csv&state='+requestData+'&pgwrap=n'
        $http.jsonp($sce.trustAsResourceUrl(requestUrl), jsonpCallbackParam: 'callback')
          .then (response) ->
            response
          , (response) ->
