  ...
  .factory 'ZuriService', [
    ...
    ($rootScope, $http, $sce) ->
      ...
      getSchoolData: (requestData, callback) ->
        if $rootScope.tablePrefix is 'heartdev'
          url = '//hearttools.heart.org/aha_ym21_dev/api/school/' + requestData + '/meta?key=k7wvZXDpmDpenVcp'
        else if $rootScope.tablePrefix is 'heartnew'
          url = '//hearttools.heart.org/aha_ym21_testing/api/school/' + requestData + '/meta?key=XgUnZxvFcjZ4jEMT'
        else
          url = '//hearttools.heart.org/aha_ym21/api/school/' + requestData + '/meta?key=XgUnZxvFcjZ4jEMT'
        $http.jsonp($sce.trustAsResourceUrl(url), jsonpCallbackParam: 'callback')
          .then (response) ->
            if response.data.success is false
              callback.error response
